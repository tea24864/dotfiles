    ...
    homeConfigurations."ubuntu" = home-manager.lib.homeManagerConfiguration {
      ...
      modules = [
        ...
        ./home.nix 
        {
          ...
          # herdr is deliberately NOT installed here: it ships its own updater
          # (`herdr update`, stable/preview channels) which installs to
          # ~/.local/bin/herdr - ahead of ~/.nix-profile/bin on PATH, so a Nix
          # copy is permanently shadowed. Home Manager manages its config only.
        }
      ];
    };
  };
